...
def ticketUltimatePending(request):
		...
		for x in serializer.data:
			...
			userPictureSe = ProfileSerializer(userPicture, many=True)
			x.update({'user_picture':userPictureSe.data[0]['pictures']})

		for y in serializers.data:
                        userPicture = Profile.objects.all().filter(user_id=y['order']['user']['id'])
                        userPictureSe = ProfileSerializer(userPicture, many=True)
                        y.update({'user_picture':userPictureSe.data[0]['pictures']})
